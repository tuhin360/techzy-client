...
const Cart = () => {
  ...
  // Local state for real-time quantity updates
  const [localCart, setLocalCart] = useState([]);

  // Sync local cart with fetched cart data
  useEffect(() => {
    setLocalCart(cart);
  }, [cart]);

  // Calculate total items (sum of all quantities) from local state
  const totalItems = localCart.reduce((total, item) => total + (item.quantity || 1), 0);

  // Calculate total price from local state
  const totalPrice = localCart.reduce(
    (total, item) => total + item.price * (item.quantity || 1),
    0
  );

  // Handle increase quantity with optimistic update
  const handleIncreaseQuantity = async (id) => {
    try {
      ...
      const res = await axiosSecure.patch(`/carts/${id}`, {
  ...
  // Handle decrease quantity with optimistic update
  const handleDecreaseQuantity = async (id) => {
    try {
      const item = localCart.find((cartItem) => cartItem._id === id);
      const currentQuantity = item.quantity || 1;

      if (currentQuantity <= 1) {
        // If quantity is 1, ask to remove item instead
        Swal.fire({
          title: "Remove Item?",
          text: "Quantity cannot be less than 1. Do you want to remove this item?",
          icon: "question",
          showCancelButton: true,
          confirmButtonColor: "#f43f5e",
          cancelButtonColor: "#9ca3af",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setLocalCart((prevCart) => prevCart.filter((item) => item._id !== id));
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      {localCart.length === 0 ? (
        ...
        <>
          {/* Cart Summary - Real-time dynamic totals */}
          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 border border-orange-200 rounded-2xl p-6 mb-8 shadow-sm">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                <div className="text-center sm:text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {localCart.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-orange-600 transition-all duration-300">
                        ৳{((item.price || 0) * (item.quantity || 1)).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-1">
                        ...
                        <span className="w-6 text-center font-medium text-sm transition-all duration-300">
                          {item.quantity || 1}
                        </span>
                        <button
                          onClick={() => handleIncreaseQuantity(item._id)}
